...
class NewsDataset(Dataset):
    def __init__(self, dataframe: pd.DataFrame) -> None:
        self.texts = dataframe["content"].astype(str).tolist()
...
def save_plots(y_true: np.ndarray, y_scores: np.ndarray, y_pred: np.ndarray, out_dir: str) -> dict:
    ...
    ax.set_title("Confusion Matrix - News Model")
    fig.tight_layout()
    ...
    ax.set_title("ROC Curve - News Model")
    fig.tight_layout()
    ...
    ax.set_title("Precision-Recall Curve - News Model")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_ylabel("Score")
    ax.set_title("Threshold vs Precision/Recall/F1 - News Model")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_title("Class Distribution (Test Set) - News")
...
def run_evaluation(args: argparse.Namespace) -> None:
    ...
    test_df = test_df.dropna(subset=["content", "label"]).copy()
    test_df["content"] = test_df["content"].astype(str)
    test_df["label"] = test_df["label"].astype(int)

    # Load tokenizer + model architecture, then load trained weights.
    ...
    # Batched inference.
    with torch.no_grad():
        for texts, labels in tqdm(loader, desc="Evaluating News", unit="batch"):
            encoded = tokenizer(
                list(texts),
                return_tensors="pt",
                truncation=True,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate News RoBERTa model with metrics and plots.")
    parser.add_argument("--test-csv", default="data/processed/news/test.csv")
    parser.add_argument("--model-path", default="models/news_model/roberta_news_best.pt")
    parser.add_argument("--model-name", default="roberta-large")
    parser.add_argument("--output-dir", default="evaluation/news")
